...
im = Image.open("icon2.ico")
st.set_page_config(
        page_title="Enigma Research",
        page_icon=im,
        layout="wide",
    )
...
def General_Market():
    ab=nse.get_index_quote('nifty 50')
    cd=nse.get_index_quote('nifty bank')
    value_1=ab['lastPrice']
    value_2=cd['lastPrice']
    delta_1=str(ab['pChange'])+"%"
    delta_2=str(cd['pChange'])+"%"
    col1, col2 = st.columns(2)
    col1.metric("NIFTY BANK", value_1, delta_1)
    col2.metric("NIFTY 50", value_2, delta_2)

    st.subheader('Top Gainers of the day(only showing stocks with tradable option contracts)')

    top_G=nse.get_top_fno_gainers()
    top_G_df=pd.DataFrame(top_G)
    top_G_df=top_G_df.loc[:4,['symbol','previousPrice','netPrice']]
    len=top_G_df.shape[0]
    if len>4:
        col1,col2,col3,col4,col5=st.columns(5)
        col1.metric(top_G_df.iloc[0,0], top_G_df.iloc[0,1], str(top_G_df.iloc[0,2])+"%")
        col2.metric(top_G_df.iloc[1,0], top_G_df.iloc[1,1], str(top_G_df.iloc[1,2])+"%")
        col3.metric(top_G_df.iloc[2,0], top_G_df.iloc[2,1], str(top_G_df.iloc[2,2])+"%")
        col4.metric(top_G_df.iloc[3,0], top_G_df.iloc[3,1], str(top_G_df.iloc[3,2])+"%")
        col5.metric(top_G_df.iloc[4,0], top_G_df.iloc[4,1], str(top_G_df.iloc[4,2])+"%")
        ...
        st.text("CURRENTLY THERE AREN'T ANY EQUITIES HAVE A NET POSITIVE PRICE CHANGE")


    st.subheader('Top Losers of the day(only showing stocks with tradable option contracts)')

    top_L=nse.get_top_fno_losers()
    top_L_df=pd.DataFrame(top_L)
    top_L_df=top_L_df.loc[:4,['symbol','previousPrice','netPrice']]
    len=top_L_df.shape[0]

    if len>4:
        col1,col2,col3,col4,col5=st.columns(5)
        col1.metric(top_L_df.iloc[0,0], top_L_df.iloc[0,1], str(top_L_df.iloc[0,2])+"%")
        col2.metric(top_L_df.iloc[1,0], top_L_df.iloc[1,1], str(top_L_df.iloc[1,2])+"%")
        col3.metric(top_L_df.iloc[2,0], top_L_df.iloc[2,1], str(top_L_df.iloc[2,2])+"%")
        col4.metric(top_L_df.iloc[3,0], top_L_df.iloc[3,1], str(top_L_df.iloc[3,2])+"%")
        col5.metric(top_L_df.iloc[4,0], top_L_df.iloc[4,1], str(top_L_df.iloc[4,2])+"%")
        ...
        st.text("CURRENTLY THERE AREN'T ANY EQUITIES HAVE A NET NEGATIVE PRICE CHANGE")        
            

    st.sidebar.markdown('---')
    st.sidebar.write('All of the data here is live(if the markets are open).')
    st.sidebar.write('To refresh the data,refresh the webpage.')
    st.sidebar.markdown('---')
    st.sidebar.write('Developed by Siddharth Kumar')
    st.sidebar.write('Contact at mm21b061@smail.iitm.ac.in ')
    
    
    st.subheader('Sector Wise Market Condition')
...
def long():
    ...
    with st.sidebar.form('settings_form'):
        ...
        mav1 = st.number_input('Moving Average 1', min_value=3, max_value=300, value=3, step=1)
        mav2 = st.number_input('Moving Average 2', min_value=3, max_value=300, value=6, step=1)
...
def Indi():
    ...
    ab['% Price Change']=str(ab['pChange'])+"%"
    ...
    st.plotly_chart(fig)
page = st.sidebar.selectbox('Select page',['General Market State','Intraday Individual Stock Analysis','Long Term Charts']) 
if page == 'General Market State':
        General_Market()

elif page == 'Intraday Individual Stock Analysis' :
        Indi()
        
elif page == 'Long Term Charts':
        long()        
